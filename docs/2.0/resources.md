...
```bash
bin/rails generate model car make kms:integer
```

Running this command will generate the expected rails files and extra the avo car controller and resource.

Car resource auto-generated by the model generation would look like this:
...
This behavior can be ommited by using the argument `--without_resource`, for example if we want to generate the model car but without the avo files we should use the following command:

```bash
bin/rails generate model car make kms:integer --without_resource
```
## Defining Resources
...
It is also possible to specify the resource model class. For example, if we want to create a new resource named MiniPost using the post model we can acchive that using the following command:

```ruby
bin/rails generate avo:resource mini-post --model_class post
```

This command will create a new resource with the same attributes as the post resource above with one single diference:
