...
You can hide that label using CSS in your [asset pipeline](./../asset-handling.html), or in an ejected [`_head` partial](./../eject-views#prepared-templates).

Avo is littered with great `data` selec˛tors so you can pick and choose any element you'd like. If it doesn't have it, we'll add it.
...
```css
[data-resource-name="TeamResource"]
  [data-field-type="external_image"][data-field-id="logo"]
  [data-slot="label"] {
  display: none;
}
```
