...
- RubyLLM 2.0, installed for you as a dependency. It is a release candidate today (`2.0.0.rc4`), which is why the gemspec asks for `>= 2.0.0.rc4`, `< 2.1` rather than `~> 2.0` — a `~>` requirement will not resolve a prerelease, and the ceiling keeps your app off a 2.1 the gem has never been run against.
- PostgreSQL
