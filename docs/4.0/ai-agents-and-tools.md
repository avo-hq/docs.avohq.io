...
| `count_records`              | Counts records — a total, per value, per day, week or month, distinct values, or within one parent record — with the same filters, scopes and search as `active_record_query`. Returns numbers only, never a row | read-only         |
