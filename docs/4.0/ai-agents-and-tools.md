...
| `mcp_find_tools`             | Searches the tools offered by the [MCP servers connected to this admin](./ai.html#connect-other-tools-with-mcp), or lists one connection's, and returns their argument schemas | read-only         |
| `mcp_call`                   | Calls one tool on a connected MCP server                                                                             | after you approve³ |
...
³ A remote call is the one proposal that leaves your application, so it takes the strictest path of any of them. `mcp_call` never contacts the server on a first call: it records the proposal and returns a card naming the connection and its real host, the remote account the call goes out as, the tool, and the exact arguments about to be sent, with **Approve once**, **Always allow**, and **Decline**. The call then runs in the background job that answers the chat, not in the click's own request, and the model sees a result only once it has. Three things make this card stricter than the others: it is settled **by button only** (typing "do it" confirms every other card and never this one, because a remote result is text an outside system wrote and can contain those words), a new message from you supersedes every pending one, and one nobody answered expires after an hour. A tool you always-allowed runs with no card at all, at most ten times in a row per reply, and stops doing so the moment the connection's owner locks that tool or the server changes the tool's name, description, or arguments. Declining sends nothing and reaches the model as a declined result, so it can carry on with the rest of the request or explain what it couldn't do. See [Connect other tools with MCP](./ai.html#connect-other-tools-with-mcp).

However many MCP servers are connected, the roster still ends at these two. Each remote tool is reached *through* them rather than added beside them, which is what keeps an app with four connections answering Avo-only questions exactly as fast as an app with none, and what lets a remote tool called `create_record` coexist with the one above it.

`mcp_find_tools` and `mcp_call` are the two exceptions to "replace one with a copy": they can be excluded like any other tool, which takes the capability away, but they can't be ejected. A copy your app owns could drop the confirmation card that stands between the model and somebody else's system.
