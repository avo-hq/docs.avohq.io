...
| `active_record_query`        | Runs read-only, paginated, policy-scoped queries against a resource — filters, scopes, grouping, and free-text search matched word by word across the text columns the resource's fields expose to the user (it does not run the resource's own `self.search` block, which could match on a column the policy withholds) | read-only         |
