...
```json [VS Code]
// Select "Install in VS Code" on the connect page, or add this to .vscode/mcp.json
{
  "servers": {
    "acme-admin": {
      "type": "http",
...
- What the client may do: **Records** as *Read*, *Read & write*, or *Choose per resource*, and **Run actions** as its own toggle. Read & write includes deleting — the word is marked in the description. Ticking run actions reveals a note that some actions may be destructive.
