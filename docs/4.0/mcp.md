...
- What the client may do: **Records** as *Read*, *Read & write*, or *Choose per resource*, and **Run actions** as its own toggle. Read & write includes deleting — the word is marked in the description. Ticking run actions reveals a quiet note that some actions may be destructive and run as soon as the client calls them.
