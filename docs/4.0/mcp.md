...
- **A pair of marks** — the client on the left, your panel's own logomark on the right. The right one is `Avo.configuration.appearance.logomark` (and `logomark_dark` when you set one), so it's your branding, not Avo's, wherever you've configured it.
  ...
  The favicon is probed server-side and the result cached for 12 hours, misses included, so a host without one is asked once rather than on every render — and a favicon host being down can never take the consent screen with it. Images are fetched with `referrerpolicy="no-referrer"`, because this page's URL carries the whole authorization request in its query string.
