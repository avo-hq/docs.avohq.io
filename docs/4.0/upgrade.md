...
Resource scopes now ship in the **`avo-scopes`** gem (extracted from `avo-advanced`). If you use scopes, see [Resource scopes (`avo-scopes`)](./avo-3-avo-4-upgrade#resource-scopes-avo-scopes) in the upgrade guide for the full checklist:

- Rename `avo-resource_scopes` → `avo-scopes` in your `Gemfile`
