...
| `add_slug_to_avo_mcp_server_connections`     | Only for an app whose `create_avo_mcp_server` predates the `slug` column on `avo_mcp_server_connections` — the connection's [name](./mcp.html#tell-connections-apart). Names every existing connection. A fresh install has the column in the first migration. |
