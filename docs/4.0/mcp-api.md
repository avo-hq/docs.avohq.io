...
| `add_identity_to_avo_mcp_server_connections` | Only for an app whose `create_avo_mcp_server` predates the `slug`, `software_name`, `software_version` and `user_agent` columns on `avo_mcp_server_connections` — the connection's [name and what its client reports as](./mcp.html#tell-connections-apart). Names every existing connection. A fresh install has the columns in the first migration. |
