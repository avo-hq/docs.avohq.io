...
Actions don't use a translation key. Instead, set `self.name`, `self.message`, `self.confirm_button_label`, and `self.cancel_button_label` to lambdas — Avo evaluates them on each request, so an `I18n.t` call inside follows the active locale:

```ruby
# app/avo/actions/toggle_inactive.rb
class Avo::Actions::ToggleInactive < Avo::BaseAction
  self.name = -> { I18n.t("avo.actions.toggle_inactive.name") }
  self.message = -> { I18n.t("avo.actions.toggle_inactive.message") }
  self.confirm_button_label = -> { I18n.t("avo.actions.toggle_inactive.confirm") }
  self.cancel_button_label = -> { I18n.t("avo.actions.toggle_inactive.cancel") }
end
```

The translation keys above are your own — define them wherever you like in your locale files.

When you leave these unset, the action name falls back to the humanized class name (`ToggleInactive` becomes "Toggle inactive"), and the confirmation dialog uses Avo's built-in `avo.run`, `avo.cancel`, and `avo.are_you_sure_you_want_to_run_this_option` keys.
