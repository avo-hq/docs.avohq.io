...
Tab and panel titles follow the same resource-scoped convention. For `tab title: "Activity"` and `panel title: "Contact information"` on a User resource, Avo looks up:

- `avo.resource_translations.user.tabs.activity`
- `avo.resource_translations.user.panels.contact_information`

The configured `title` remains the fallback when the key is missing. The suffix is the title parameterized with underscores (`"Contact information"` → `contact_information`).
...
Pass a full `translation_key:` on the `tab` or `panel` when the generated key is not suitable:

```ruby
tab title: "Activity", translation_key: "avo.resource_translations.user.tabs.timeline" do
  field :comments, as: :has_many
end
```

Callable titles still work — Avo evaluates them in the resource execution context, then uses the resolved string for both display (as the fallback) and the parameterized lookup key.
