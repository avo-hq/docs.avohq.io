...
| --- | --- | --- | --- |
| [Resource](#localizing-resources) | `avo.resource_translations.<class_path>` | `zero`/`one`/`other`, `description`, plus the `fields`, `tabs`, `panels` and `save` subtrees | Avo |
...
`description` fills the resource's [`self.description`](./resources-api.html#self.description) text. Like everywhere else in the cascade, the locale file beats `self.description` on the class, so you can leave an English default in code and translate over it. The one thing to watch: if `self.description` is a block that reads `record` or `view`, don't add the key for that resource — a static translation would take its place.
...
The locale generator covers Avo core. Add-on gems keep their own strings under their own namespace inside `avo.*`. Most of them ship **English only** — you supply the other locales in your app, the same way you would for any other key. A gem that ships more says so on its own Localization page: `avo-api` ships its tree in every locale core does, so the REST API's panel is translated with no work from you.
...
| REST API | `avo.api.token.*` | [Localization](./rest-api.html#localization) |
