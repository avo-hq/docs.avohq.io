...
`$ rails g avo:eject --field-components <field_type>`

Replace `<field_type>` with the desired field type. For instance, to eject components for a Trix field, use:

`$ rails g avo:eject --field-components trix`

This command will generate the files for both the edit and show components of the Trix field, for each field type the amount of components may vary.
...
Here's an example of how to use the `components` option in a field definition:
...
field :description,
  ...
  components: -> do
    {
      field_show_component: Avo::Fields::Admin::TrixField::ShowComponent
    }
  end
```
...
`field_<view>_component` is the key used to render the field's `<view>`'s component, replace `<view>` with one of the views in order to customize a component per each view.
