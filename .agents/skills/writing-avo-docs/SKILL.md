...
with a **two-page model**: a task-oriented _guide_ and an exhaustive _reference
(API)_. Your job is to produce pages that match that model exactly, so the docs
...
matters is _guide vs. reference_, never _topic vs. topic_. A reader should be
...
This page is for _consulting_, not reading top to bottom.
...
- [ ] Guide is task-organized, plain English, skimmable; opens with description + common-case code + defaults note.
...
Whenever an add_on_link is provided check the URL to see if there's an addon selected.
