...
const { MarkdownUtils } = require('./markdown-processor.js');
...
class TemplateGenerator {
   ...
   * @param {Object} options - Generation options
   * @returns {Promise<Object>} Collection of generated sections
   */
  async generateSections(processedFiles, options = {}) {
    // Filter out everything.md files to prevent duplicate output
    const filteredFiles = processedFiles.filter(file => {
      const fileName = path.basename(file.relativePath || file.filePath || '');
      return fileName !== 'everything.md';
    });

    this.logger.info('Generating multiple section templates', {
      totalFiles: processedFiles.length,
      filteredFiles: filteredFiles.length,
      excludedFiles: processedFiles.length - filteredFiles.length
    });
    ...
    for (const processedFile of filteredFiles) {
      try {
        const section = await this.generateSection(processedFile, options);
  ...
  formatForLLMs(section, options) {
    ...
    // Add title header only if content doesn't already contain it at the beginning
    if (section.metadata?.title) {
      const expectedTitle = `# ${section.metadata.title}`;
      const contentTrimmed = section.content.trim();

      // Check if the content already starts with this exact title or any h1 heading
      const startsWithTitle = contentTrimmed.startsWith(expectedTitle);
      const startsWithAnyH1 = /^#\s+/.test(contentTrimmed);

      if (!startsWithTitle && !startsWithAnyH1) {
        output += `${expectedTitle}\n\n`;
      }
    }

    // Add metadata comment
    if (options.includeMetadata) {
      output += `<!-- Source: ${section.metadata?.sourceFile || 'Unknown'}`;
  ...
  combineSections(sections, options = {}) {
    ...
    const combined = {
      metadata: {
        ...
        sources: sections.map(s => s.metadata?.sourceFile || 'Unknown source'),
        generatedAt: new Date().toISOString()
      },
      sections: sections.map(s => ({
        id: s.id,
        title: s.metadata?.title || 'Untitled',
        source: s.metadata?.sourceFile || 'Unknown source'
      })),
      content: ''
    };
 ...
 */
function generateSection(processedFile, config = {}) {
