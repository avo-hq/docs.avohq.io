...
function extractSidebarFromConfig(sidebarConfig) {
  ...
  try {
    ...
    for (const [versionPath, sectionArray] of Object.entries(sidebarConfig)) {
      ...
      // Non-versioned sidebar sections (e.g. "/contributing/") are intentional
      // and simply skipped — they don't belong to any docs version.
      const versionMatch = versionPath.match(/\/(\d+\.\d+)\//);
      if (!versionMatch) {
        logger.debug(`Skipping non-versioned sidebar section: ${versionPath}`);
        continue;
      }
